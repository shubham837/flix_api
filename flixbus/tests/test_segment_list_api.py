...
class SegmentListTest(TestCase):
    ...
    def test_get_segment_list(self):
        resp = self.client.get('/segment?start=40&get_pax=true',
                               headers=self.headers,
                               content_type='application/json')
        get_segment_response_data = json.loads(resp.data)
        self.assertEquals(resp.status_code, 200)
        self.assertEquals(len(get_segment_response_data), 6)

    def test_post_segment_list(self):
        segment_body = {
            "from_stop": 7,
            "destination_stop": 10,
            "is_active": True
        }
        resp = self.client.post('/segment', headers=self.headers,
