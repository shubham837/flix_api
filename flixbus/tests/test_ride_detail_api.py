...
class RideDetailTest(TestCase):
    ...
    def test_get_ride_detail(self):
        resp = self.client.get('/route/1/ride/1', headers=self.headers,
                               content_type='application/json')
        route_response_data = json.loads(resp.data)
        self.assertEquals(resp.status_code, 200)
        self.assertEquals(route_response_data.get('id'), 1)

    def test_put_ride_detail(self):
        ride_body = {
            "route_id": 1,
            "from_stop": 1,
            "destination_stop": 2
        }
        resp = self.client.put('/route/1/ride/1', headers=self.headers,
                               data=json.dumps(ride_body),
                               content_type='application/json')
        self.assertEquals(resp.status_code, 200)
