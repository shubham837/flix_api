...
class RouteListTest(TestCase):
    ...
    def test_get_route_list(self):
        resp = self.client.get('/route', headers=self.headers,
                               content_type='application/json')
        get_segment_response_data = json.loads(resp.data)
        self.assertEquals(resp.status_code, 200)
        self.assertEquals(len(get_segment_response_data), 4)

    def test_post_route_list(self):
        route_body = {
            "route_segments": [
                {
                    "segment_id": 1,
        ...
        resp = self.client.post('/route', headers=self.headers,
