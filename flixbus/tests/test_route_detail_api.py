...
class RouteDetailTest(TestCase):
    ...
    def test_get_route_detail(self):
        resp = self.client.get('/route/1', headers=self.headers,
                               content_type='application/json')
        route_response_data = json.loads(resp.data)
        self.assertEquals(resp.status_code, 200)
        self.assertEquals(route_response_data.get('route_id'), 1)

    def test_put_route_detail(self):
        route_body = {
            "route_segments": [
                {
                    "segment_id": 1,
        ...
        resp = self.client.put('/route/1', headers=self.headers,
